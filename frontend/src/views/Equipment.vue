...
const fetchData = async () => {
  ...
  const result: any = await equipmentApi.getList(params)
...
const confirmDelete = async () => {
  if (deleteId.value) {
    await equipmentApi.delete(deleteId.value)
...
<template>
  <div class="space-y-4">
    <div class="flex items-center justify-between">
      <h2 class="text-xl font-bold text-fire-dark">器材管理</h2>
      <el-button type="primary" @click="router.push('/equipment/add')">
      ...
      <el-table-column label="操作" width="180" fixed="right">
        <template #default="{ row }">
          <div class="flex gap-2">
            <el-button size="small" @click="router.push(`/equipment/edit/${row.id}`)">
              <Edit class="w-4 h-4" />
            </el-button>
