...
教员按班组和测验日记折返跑测验的**应测人数、合格人数**，解决“纸上改过人数、总览合格率对不上当天改后人数”的问题。

业务规则：

- **同一班组同一测验日只保留一行成绩**：第一次记为新增，当天改人数走同一行更新，不会出现两组成绩。
- **合格率不入库**：班组页和训练基地总览上的合格率都在读取时按 `合格人数 ÷ 应测人数` 实时算出，共用同一个后端接口口径，所以改完合格人数两处一起重算，数字必然一致——只加列、总览停在旧率不算做完。
- **应测人数改小到低于已写下的合格人数时，本次改人数不能保存**（返回 400，提示“合格人数比应测人数还多”），整单回滚：人数不变，班组卡片与总览两处合格率都仍是保存前的值。
- **两个教员同时改同一班组同一日的合格人数**：成绩行带 `version` 乐观锁，先提交者生效，后提交者整单返回 409，不会把先写成功的人数盖掉；刷新后按最新人数重改即可。

接口：

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | /api/shuttle-run/overview?testDate= | 某测验日全部班组的应测/合格/实时合格率（未记成绩的班组为空） |
| GET | /api/shuttle-run/summary?testDate= | 某测验日总应测、总合格、总合格率 |
| GET | /api/shuttle-run/team/{teamId} | 某班组各测验日成绩（按测验日倒序） |
| GET | /api/shuttle-run/team/{teamId}/date/{testDate} | 某班组某测验日唯一一行（无则 204） |
| PUT | /api/shuttle-run/team/{teamId}/date/{testDate} | 按班组+测验日整单保存（合格多于应测返回 400，并发被抢先返回 409，均不落库） |
